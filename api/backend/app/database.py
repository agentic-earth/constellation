...
def connect_supabase() -> Client:
    ...
    try:
        ...
        if not url or not key:
            raise ValueError(
                "SUPABASE_URL and SUPABASE_KEY must be set in configuration."
            )

        logger.log(
            "Database", "info", f"Attempting to connect to Supabase with URL: {url}"
        )
        ...
        logger.log(
            "Database", "critical", f"Failed to connect Supabase Client: {str(e)}"
        )
        raise
...
def connect_supabase_admin() -> Client:
    try:
        ...
        if not url or not service_key:
            raise ValueError(
                "SUPABASE_URL and SUPABASE_SERVICE_KEY must be set in configuration."
            )

        logger.log(
            "Database",
            "info",
            f"Attempting to connect to Supabase Admin with URL: {url}",
        )
        ...
        logger.log(
            "Database", "critical", f"Failed to connect Supabase Admin Client: {str(e)}"
        )
        raise
...
async def connect_db():
    try:
        ...
        logger.log("Database", "info", "Prisma Client connected successfully.")
    except Exception as e:
        logger.log(
            "Database",
            "critical",
...
async def disconnect_db():
    try:
        ...
        logger.log("Database", "info", "Prisma Client disconnected successfully.")
    except Exception as e:
        logger.log(
            "Database",
            "error",
