...
class UserService:
    ...
    async def register_user_email_password(
        self, prisma: Prisma, supabase: Client, email: str, password: str, username: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing_user = await self.get_profile_by_email(prisma, email)
            if existing_user:
                self.logger.log(
                    "UserService",
                    "warning",
                    "Attempted to register existing email",
                    email=email,
                )
                return {
                    "success": False,
            ...
            existing_username = await self.get_profile_by_username(prisma, username)
            ...
            if not user:
                self.logger.log(
                    "UserService",
                    "error",
                    "No user returned from Supabase Auth sign up",
                )
                return {
                    "success": False,
                    "error": "auth_error",
            ...
            if not auth_response.session:
                self.logger.log(
                    "UserService",
                    "info",
                    "User registered but email confirmation is required",
                    user_id=user.id,
                )
                return {
                    "success": True,
            ...
            profile = await prisma.profile.find_unique(where={"auth_uid": user.id})
            if profile:
                self.logger.log(
                    "UserService",
                    "info",
                ...
                self.logger.log(
                    "UserService",
                    "error",
                    "Profile not found after user registration",
                    user_id=user.id,
                )
                return {
                    "success": False,
                    "error": "profile_not_found",
                    "message": "User account created but profile not found.",
                }
            ...
            self.logger.log(
                "UserService",
                "error",
                "Failed to register user via email/password",
                error=str(e),
    ...
    async def sign_in_user_email_password(
        self, supabase: Client, email: str, password: str
    ) -> Dict[str, Any]:
        try:
            ...
            if auth_response.user and auth_response.session:
                ...
                self.logger.log(
                    "UserService",
                    "error",
                    "Sign in failed: No user or session returned",
                )
                return {
                    "success": False,
                    "error": "auth_error",
            ...
            self.logger.log(
                "UserService",
                "error",
                "Failed to sign in user via email/password",
                error=str(e),
                traceback=traceback.format_exc(),
            )
    ...
    ) -> Optional[PrismaProfile]:
        """
        Retrieves a user's Profile by their userId (auth_uid).
        """
        try:
            profile = await prisma.profile.find_unique(where={"auth_uid": str(user_id)})
            if profile:
                self.logger.log(
                    "UserService",
                    "info",
            ...
            self.logger.log(
                "UserService",
                "error",
                "Failed to retrieve profile by userId",
                error=str(e),
                traceback=traceback.format_exc(),
            )
            return None
    ...
    async def get_profile_by_email(
        self, prisma: Prisma, email: str
    ) -> Optional[PrismaProfile]:
        """
        Retrieves a user's Profile by their email.
        """
        try:
            profile = await prisma.profile.find_unique(where={"email": email})
            if profile:
                self.logger.log(
                    "UserService",
                    "info",
                ...
                self.logger.log(
                    "UserService", "warning", "Profile not found for email", email=email
                )
            return profile
        except Exception as e:
            self.logger.log(
    ...
    async def get_profile_by_username(
        self, prisma: Prisma, username: str
    ) -> Optional[PrismaProfile]:
        """
        Retrieves a user's Profile by their username.
        """
        try:
            profile = await prisma.profile.find_unique(where={"username": username})
            if profile:
                self.logger.log(
                    "UserService",
                    "info",
    ...
    async def update_profile(
        self, prisma: Prisma, user_id: UUID, update_data: Dict[str, Any]
    ) -> Optional[PrismaProfile]:
        """
        ...
        try:
            ...
            updated_profile = await prisma.profile.update(
                where={"auth_uid": str(user_id)}, data=update_data
            )
            self.logger.log(
                "UserService",
                "info",
                "Profile updated successfully",
                user_id=updated_profile.auth_uid,
            )
    ...
    async def delete_user(self, supabase_admin: Client, user_id: UUID) -> bool:
        """
        ...
        try:
            ...
            auth_response = supabase_admin.auth.admin.delete_user(str(user_id))

            if auth_response.error:
                self.logger.log(
                    "UserService",
                    "error",
                    "Supabase Auth delete failed",
                    error=auth_response.error.message,
                )
                return False
            ...
            self.logger.log(
                "UserService",
                "info",
                "User deleted successfully from Supabase Auth",
                user_id=str(user_id),
            )
            return True
...
async def main():
    ...
    # Note: Since we're passing clients to methods, we only need to pass logger and hash_salt here
    hash_salt = "your_hash_salt_here"  # Replace with your actual hash salt or retrieve from environment
    user_service = UserService(logger, hash_salt)

    # Test user registration
    email = "mason_lee@brown.edu"
    password = (
        "securePassword123!"  # In a real scenario, this should be provided by the user
    )
    username = "mason_lee"

    try:
        print("=== Starting User Registration ===")
        registration_result = await user_service.register_user_email_password(
    ...
    try:
        ...
        sign_in_result = await user_service.sign_in_user_email_password(
            supabase=supabase_client, email=email, password=password
        )
        if sign_in_result["success"]:
            ...
            try:
                ...
                profile = await user_service.get_profile_by_user_id(
                    prisma=prisma_client, user_id=UUID(user_id)
                )
                if profile:
            ...
            try:
                ...
                updated_profile = await user_service.update_profile(
                    prisma=prisma_client, user_id=UUID(user_id), update_data=update_data
                )
                if updated_profile:
            ...
            try:
                ...
                delete_result = await user_service.delete_user(
                    supabase_admin=supabase_admin_client, user_id=UUID(user_id)
                )
                if delete_result:
            ...
            print(
                f"User sign-in failed: {sign_in_result.get('message', 'Unknown error')}"
            )
    except Exception as e:
