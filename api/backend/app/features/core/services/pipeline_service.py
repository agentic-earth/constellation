...
from prisma.models import (
    ...
    PipelineBlock as PrismaPipelineBlock,
    PipelineEdge as PrismaPipelineEdge,
)
...
class PipelineService:
    ...
    async def create_pipeline(
        self, tx: Prisma, pipeline_data: Dict[str, Any]
        ...
        try:
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Unique violation when creating pipeline.",
                error=str(e),
            )
            return None
    ...
    async def get_pipeline_by_id(
        self, tx: Prisma, pipeline_id: UUID
    ) -> Optional[PrismaPipeline]:
        """
        ...
        try:
            pipeline = await tx.pipeline.find_unique(
                ...
                include={
                    "PipelineBlock": {"include": {"Block": True}},
                    "PipelineEdge": {"include": {"Edge": True}},
                },
            )
            if pipeline:
                self.logger.log(
            ...
            self.logger.log(
                "PipelineService", "error", "Error retrieving pipeline.", error=str(e)
            )
            return None

    async def update_pipeline(
        self, tx: Prisma, pipeline_id: UUID, update_data: Dict[str, Any]
    ) -> Optional[PrismaPipeline]:
        """
        ...
        Args:
            ...
            update_data (Dict[str, Any]): Dictionary containing updated pipeline data.
                Allowed keys: 'name', 'description', 'run_id'.
        ...
        try:
            ...
            updated_pipeline = await tx.pipeline.update(
                where={"pipeline_id": str(pipeline_id)}, data=update_data
            )
            self.logger.log(
                "PipelineService",
                "info",
            ...
            self.logger.log(
                "PipelineService", "error", "Error updating pipeline.", error=str(e)
            )
            return None
    ...
    async def delete_pipeline(self, tx: Prisma, pipeline_id: UUID) -> bool:
        ...
        try:
            ...
            self.logger.log(
                "PipelineService", "error", "Error deleting pipeline.", error=str(e)
            )
            return False

    async def assign_version_to_pipeline(
        self, tx: Prisma, pipeline_id: UUID, version_id: UUID
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error assigning version to pipeline.",
                error=str(e),
            )
            return False
    ...
    async def search_pipelines(
        ...
        try:
            ...
            self.logger.log(
                "PipelineService", "error", "Error searching pipelines.", error=str(e)
            )
            return []
    ...
    async def list_pipelines(
        ...
        try:
            ...
            self.logger.log(
                "PipelineService", "error", "Error listing pipelines.", error=str(e)
            )
            return []

    async def assign_block_to_pipeline(
        self, tx: Prisma, block_id: UUID, pipeline_id: UUID
    ) -> Optional[PrismaPipelineBlock]:
        """
        ...
        try:
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error assigning block to pipeline.",
                error=str(e),
            )
            return None

    async def assign_edge_to_pipeline(
        self, tx: Prisma, edge_id: UUID, pipeline_id: UUID
    ) -> Optional[PrismaPipelineEdge]:
        """
        ...
        try:
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error assigning edge to pipeline.",
                error=str(e),
            )
            return None
    ...
    async def get_pipeline_blocks(
        self, tx: Prisma, pipeline_id: UUID
    ) -> Optional[List[PrismaPipelineBlock]]:
        """
        ...
        """
        try:
            pipeline_blocks = await tx.pipelineblock.find_many(
                where={"pipeline_id": str(pipeline_id)}
            )
            self.logger.log(
                "PipelineService",
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error retrieving pipeline blocks.",
                error=str(e),
            )
            return None
    ...
    async def get_pipeline_edges(
        self, tx: Prisma, pipeline_id: UUID
    ) -> Optional[List[PrismaPipelineEdge]]:
        """
        ...
        """
        try:
            pipeline_edges = await tx.pipelineedge.find_many(
                where={"pipeline_id": str(pipeline_id)}
            )
            self.logger.log(
                "PipelineService",
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error retrieving pipeline edges.",
                error=str(e),
            )
            return None

    async def remove_block_from_pipeline(
        self, tx: Prisma, pipeline_block_id: UUID
    ) -> bool:
        try:
            deleted_pb = await tx.pipelineblock.delete(
                where={"pipeline_block_id": str(pipeline_block_id)}
            )
            self.logger.log(
                "PipelineService",
                "info",
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error removing block from pipeline.",
                error=str(e),
            )
            return False

    async def remove_edge_from_pipeline(
        self, tx: Prisma, pipeline_edge_id: UUID
    ) -> bool:
        try:
            deleted_pe = await tx.pipelineedge.delete(
                where={"pipeline_edge_id": str(pipeline_edge_id)}
            )
            self.logger.log(
                "PipelineService",
                "info",
            ...
            self.logger.log(
                "PipelineService",
                "error",
                "Error removing edge from pipeline.",
                error=str(e),
            )
            return False
    ...
    async def main(self):
        ...
        try:
            async with prisma.tx() as tx:
                ...
                if created_pipeline:
                    print(
                        f"Pipeline created: {created_pipeline.pipeline_id} - {created_pipeline.name}"
                    )
                else:
                ...
                if created_pipeline:
                    ...
                    fetched_pipeline = await self.get_pipeline_by_id(
                        tx, UUID(created_pipeline.pipeline_id)
                    )
                    if fetched_pipeline:
                        print(
                            f"Fetched Pipeline: {fetched_pipeline.pipeline_id} - {fetched_pipeline.name}"
                        )
                    else:
                ...
                if created_pipeline:
                    ...
                    updated_pipeline = await self.update_pipeline(
                        tx, UUID(created_pipeline.pipeline_id), update_data
                    )
                    if updated_pipeline:
                        print(
                            f"Updated Pipeline: {updated_pipeline.pipeline_id} - {updated_pipeline.name}"
                        )
                    else:
                ...
                if created_pipeline:
                    ...
                    )  # Replace with actual version ID
                    assign_success = await self.assign_version_to_pipeline(
                        tx, UUID(created_pipeline.pipeline_id), version_id
                    )
                    print(f"Version assigned: {assign_success}")

                # Step 5: Search for pipelines
                print("\nSearching for pipelines with name containing 'Updated'...")
                search_params = {"name": "Updated"}
                ...
                if created_pipeline:
                    ...
                    deletion_success = await self.delete_pipeline(
                        tx, UUID(created_pipeline.pipeline_id)
                    )
            ...
            self.logger.log(
                "PipelineService", "error", "An error occurred in main.", error=str(e)
            )
