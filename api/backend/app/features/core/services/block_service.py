...
class BlockService:
        ...
        if vector:
            try:
                ...
                self.logger.log(
                    "BlockService",
                    "info",
                    f"Vector created for block {created_block.block_id}",
                )
            except Exception as e:
                self.logger.log(
                    "BlockService",
                    "error",
                    f"Failed to set vector",
                    error=str(e),
                    traceback=traceback.format_exc(),
                )
                return None
    ...
    async def get_block_by_id(
        ...
        """
        try:
            block = await tx.block.find_unique(
                where={"block_id": str(block_id)}, include={"paper": True}
            )
            ...
            self.logger.log(
                "BlockService", "error", "Failed to retrieve block by ID", error=str(e)
            )
            return None
    ...
    async def get_block_by_name(self, tx: Prisma, name: str) -> Optional[PrismaBlock]:
        ...
        try:
            block = await tx.block.find_unique(
                where={"name": name}, include={"paper": True}
            )
            ...
            self.logger.log(
                "BlockService",
                "error",
                "Failed to retrieve block by name",
                error=str(e),
            )
            return None
    ...
    async def update_block(
        ...
        try:
            ...
            updated_block = await tx.block.update(
                where={"block_id": str(block_id)}, data=update_data
            )
            ...
            self.logger.log(
                "BlockService", "error", "Failed to update block", error=str(e)
            )
            return None

        if vector:
            set_vector = await self.set_block_vector(
                tx, block_id=updated_block.block_id, vector=vector
            )
    ...
    async def delete_block(self, tx: Prisma, block_id: UUID) -> bool:
        ...
        try:
            ...
            self.logger.log(
                "BlockService", "error", "Failed to delete block", error=str(e)
            )
            return False

    async def set_block_vector(
        self, tx: Prisma, block_id: str, vector: List[float]
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.log(
                "BlockService", "info", f"Vector created for block {block_id}"
            )
            return True
        except Exception as e:
            self.logger.log(
    ...
    async def get_block_vector(
        self, tx: Prisma, block_id: str
    ) -> Optional[List[float]]:
        """
        ...
        """
        try:
            filters = {"field": "id", "operator": "==", "value": block_id}
            documents = self.document_store.filter_documents(filters=filters)
            self.logger.log(
                "BlockService",
                "info",
                f"Retrieved block vector - {documents[0].embedding[:5]}... truncated",
            )
            ...
            self.logger.log(
                "BlockService",
                "error",
                f"Failed to retrieve block vector - error={str(e)}",
            )
            return None
    ...
    async def search_blocks_by_vector_similarity(
        self, tx: Prisma, query_vector: List[float], top_k: int = 5
    ) -> List[PrismaBlock]:
        """
    ...
    async def get_all_vectors(self, tx: Prisma) -> List[List[float]]:
        ...
        try:
            ...
            self.logger.log(
                "BlockService", "info", f"Retrieved {len(vectors)} vectors for blocks."
            )
    ...
    async def get_llm_output(self, query: str, blocks: List[PrismaBlock]) -> Optional[str]:
        """
        ...
        try:
            ...
            self.logger.log(
                "BlockService", "info", f"LLM output retrieved successfully: {len(result)}."
            )
...
async def main():
    ...
    try:
        async with prisma.tx(timeout=10000) as tx:
            ...
            block2_vector = await VectorEmbeddingService().generate_text_embedding(
            ...
            created_block_with_vector = await block_service.create_block(
                tx, new_block_with_vector_data, vector=block2_vector
            )
            if created_block_with_vector:
                print(
                    f"Created block with vector: {created_block_with_vector.block_id}"
                )
            else:
            ...
            if created_block_with_vector:
                ...
                retrieved_block = await block_service.get_block_by_id(
                    tx, UUID(block_id)
                )
                ...
                block_by_name = await block_service.get_block_by_name(
                    tx, created_block_with_vector.name
                )
                ...
                updated_block = await block_service.update_block(
                    tx, UUID(block_id), update_data
                )
                ...
                update_block2_vector = await VectorEmbeddingService().generate_text_embedding(
                ...
                vector_success = await block_service.set_block_vector(
                    tx, block_id, update_block2_vector
                )
                ...
                query_vector = await VectorEmbeddingService().generate_text_embedding(
                    "languages"
                )
                similar_blocks = await block_service.search_blocks_by_vector_similarity(
                    tx, query_vector, top_k=5
                )
                print(f"Similar blocks: \n")
                print(
                    f"1. id: {similar_blocks[0]["id"]}, score: {similar_blocks[0]["score"]}\n"
                )
                print(
                    f"2. id: {similar_blocks[1]["id"]}, score: {similar_blocks[1]["score"]}"
                )
            ...
            for block in all_blocks:
                print(
                    f"- Block ID: {block.block_id}, Name: {block.name}, Type: {block.block_type}, Description: {block.description}"
                )
