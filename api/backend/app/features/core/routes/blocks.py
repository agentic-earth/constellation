...
def delete_block(block_id: UUID):
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail="Block not found or already deleted."
        )
    return
...
def assign_version(block_id: UUID, version_id: UUID):
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail="Failed to assign version to block."
        )
...
def search_blocks(
    ...
    block_type: Optional[BlockTypeEnum] = Query(
        None, description="Filter by block type"
    ),
