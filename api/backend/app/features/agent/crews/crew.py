...
class LLMCrew:
    ...
    def build_task(query: str, blocks: List[PrismaBlock], agent: Agent) -> Task:
        ...
        """
        # Blocks only show properties name, block_type, filepath, and description.
        blocks = [
            {
                "name": block.name,
        ...
        description = (
            ...
            + "\n".join(
                [
                    f"Name: {block['name']}, Block Type: {block['block_type']}, Description: {block['description']}, Filepath: {block['filepath']}"
                    for block in blocks
                ]
            )
            + """
