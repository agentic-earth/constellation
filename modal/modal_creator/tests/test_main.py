...
def test_deploy():
    ...
    assert response.json() == {"message": f"{VALID_MODEL_NAME} has been deployed succesfully!",
    ...
    assert response.status_code == 200
    assert response.json() == {"message": f"{VALID_MODEL_NAME} has already been deployed.",
                            "endpoint":  f"https://wdorji--{service_name}-flask-app.modal.run/infer",
                            "service_name": service_name}
...
def test_infer():
    ...
    response = client.post("/infer?model_name="+VALID_MODEL_NAME, json={"data": [convert_image_to_base64("modal_creator/assets/forest.jpg")]})
    assert response.status_code == 200
    assert response.json() == {"message": f"{VALID_MODEL_NAME} has not been deployed yet."}
    ...
    response = client.post("/infer?model_name="+VALID_MODEL_NAME, json={"data": [convert_image_to_base64("modal_creator/assets/forest.jpg")]})
    assert response.status_code == 200
    assert max(response.json()['output'][0], key=lambda x: x['score'])['label'] == 'Normal'
...
def test_delete():
    ...
    response = client.delete("/delete?model_name="+VALID_MODEL_NAME)
    assert response.status_code == 200
    assert response.json() == {"message": f"{VALID_MODEL_NAME} has been deleted succesfully!"}