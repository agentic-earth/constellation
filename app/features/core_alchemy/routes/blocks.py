...
from app.features.core_alchemy.controllers.block_controller import BlockController
...
async def create_block(block: BlockCreateSchema, session: AsyncSession = Depends(get_session)):
    """
    Create a new block.
    ...
    """
    created_block = await block_controller.create_block(block, session)
...
async def get_block(block_id: UUID, session: AsyncSession = Depends(get_session)):
    """
...
async def update_block(block_id: UUID, block_update: BlockUpdateSchema, session: AsyncSession = Depends(get_session)):
    """
...
async def delete_block(block_id: UUID, session: AsyncSession = Depends(get_session)):
    """
...
async def list_blocks(name: Optional[str] = None, block_type: Optional[str] = None, session: AsyncSession = Depends(get_session)):
    """
...
async def assign_version(block_id: UUID, version_id: UUID, session: AsyncSession = Depends(get_session)):
    """
