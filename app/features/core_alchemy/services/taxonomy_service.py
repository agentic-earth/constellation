...
from app.alchemy_models import TaxonomyCategory
...
class TaxonomyService:
    ...
    async def create_or_get_category(self, name: str, session: AsyncSession, parent_id: Optional[UUID] = None) -> Optional[UUID]:
        """
        ...
        try:
            ...
            if not parent_id:
                response = await session.execute(select(TaxonomyCategory.category_id).where(TaxonomyCategory.name == name))
            else:
                response = await session.execute(select(TaxonomyCategory.category_id).where(TaxonomyCategory.name == name, TaxonomyCategory.parent_id == str(parent_id)))
            
            category_id = response.scalars().first()
            if category_id:
            ...
            category_data = TaxonomyCategory(
                category_id = str(new_category_id),
                name = name,
                parent_id = str(parent_id) if parent_id else None,
                created_at = current_time.isoformat(),
                update_at = current_time.isoformat(),
            )
    ...
    async def process_taxonomy(self, taxonomy: Dict[str, Any], session: AsyncSession, parent_id: Optional[UUID] = None) -> List[UUID]:
        """
        ...
        for key, value in taxonomy.items():
            category_id = await self.create_or_get_category(name=key, parent_id=parent_id, session=session)
            if category_id:
    ...
    async def associate_block_with_categories(self, block_id: UUID, category_ids: List[UUID], session: AsyncSession) -> bool:
        """
    ...
    async def create_taxonomy_for_block(self, block_id: UUID, taxonomy: Taxonomy, session: AsyncSession) -> bool:
        """
        ...
        try:
            ...
            general_categories = await self.process_taxonomy(taxonomy=general_taxonomy, session=session)
            ...
            if specific_taxonomy:
                ...
                specific_categories = await self.process_taxonomy(taxonomy=specific_taxonomy, session=session)
                general_categories.extend(specific_categories)

            # Associate block with all processed categories
            success = await self.associate_block_with_categories(block_id=block_id, category_ids=general_categories, session=session)
            return success
    ...
    async def search_blocks_by_taxonomy(self, taxonomy_filters: Dict[str, Any], session: AsyncSession) -> Optional[List[Dict[str, Any]]]:
        """
    ...
    async def get_taxonomy_for_block(self, block_id: UUID, session: AsyncSession) -> Optional[Dict[str, Any]]:
        """
