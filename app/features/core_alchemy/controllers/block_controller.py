...
from app.features.core_alchemy.services.block_service import BlockService
from app.features.core_alchemy.services.taxonomy_service import TaxonomyService
from app.features.core_alchemy.services.vector_embedding_service import VectorEmbeddingService
from app.features.core_alchemy.services.audit_service import AuditService
...
class BlockController:
    ...
    async def create_block(self, block_data: BlockCreateSchema, session: AsyncSession) -> Optional[BlockResponseSchema]:
        """
        ...
        try:
            async with session.begin():
                ...
                block = await self.block_service.create_block(block_data, session)
                if not block:
                ...
                if block_data.taxonomy:
                    taxonomy_success = await self.taxonomy_service.create_taxonomy_for_block(block_id=block.block_id, 
                ...
                await session.commit()  # commit if all steps succeed
                return block
    ...
    async def get_block_by_id(self, block_id: UUID, session: AsyncSession) -> Optional[BlockResponseSchema]:
        """
    ...
    async def update_block(self, block_id: UUID, update_data: BlockUpdateSchema, session: AsyncSession) -> Optional[BlockResponseSchema]:
        """
    ...
    async def delete_block(self, block_id: UUID, session: AsyncSession) -> bool:
        """
        ...
        try:
            ...
            self.taxonomy_service.delete_taxonomy_for_block(block_id=block_id)
            deleted = self.block_service.delete_block(block_id=block_id)
            if not deleted:
    ...
    async def update_vector_embedding(self, block_id: UUID, update_data: VectorRepresentationCreateSchema, session: AsyncSession) -> bool:
        try:
            updated = self.vector_embedding_service.update_vector_embedding(block_id, update_data)
            if not updated:
    ...
    async def get_vector_embedding(self, block_id: UUID, session: AsyncSession) -> Optional[VectorRepresentationSchema]:
        """
    ...
    async def create_vector_embedding(self, vector_data: VectorRepresentationCreateSchema, session: AsyncSession) -> Optional[VectorRepresentationResponseSchema]:
        try:
            vector_embedding = self.vector_embedding_service.create_vector_embedding(vector_data)
            ...
            self.logger.log(
                ...
                extra={"entity_id": str(vector_data.entity_id), "entity_type": vector_data.entity_type}
            )
            return vector_embedding
            ...
            self.logger.log(
                ...
                extra={"traceback": traceback.format_exc(), "entity_id": str(vector_data.entity_id)}
            )
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal Server Error.")


    async def delete_vector_embedding(self, block_id: UUID, session: AsyncSession) -> bool:
        """
    ...
    async def perform_similarity_search(self, query_text: str, session: AsyncSession, taxonomy_filters: Optional[Dict[str, Any]] = None, top_k: int = 10) -> Optional[List[BlockResponseSchema]]:
        """
